...
import { UPDATE_PRODUCTS, UPLOAD_IMAGE_ON_CLOUD } from '../../../api/apiUrl';
...
export default function Modal({ showModal, setShowModal, getProducts, subCategories, categories, measurement, productData }) {
    ...
    const [Products, setProducts] = useState({ id: '', title: '', description: '', quantity: '', size: '', price: '', image: '', category_id: '', subcategory_id: '', measurement_id: '', image_file_name: '', low_quantity: '', isLowQuantity:false });
    ...
    const updateProduct = async () => {
        ...
        formdata.append("low_quantity", Products.low_quantity);
    ...
    useEffect(() => {
        setProducts({
            ...
            low_quantity: productData?.low_quantity ? productData.low_quantity : '',
            isLowQuantity: productData?.low_quantity > 0 ? true : false,
        });
    }, [productData]);
                        ...
                        <div className="relative w-auto my-6 mx-auto max-w-3xl">
                            <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none width-modal">
                                <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                                    <h3 className="text-xl font-semibold text-center"> {t('Update Product')}</h3>
                                ...
                                <div className="relative p-3 flex-auto modal-contant-width" ref={divRef}>
                                    ...
                                    <div className="px-3 my-2 sm:flex sm:justify-start sm:items-center">
                                        <div className="form-check px-2">
                                                <input className="form-check-input" defaultChecked={Products?.isLowQuantity} name="isLowQuantity" type="checkbox" value="" id="flexCheckDefault"  onChange={(e) => setProducts({ ...Products, [e.target.name]: e.target.checked  })}/>
                                                <label className="uppercase tracking-wide text-gray-700 text-xs font-bold ml-2 mb-2" htmlFor="flexCheckDefault">
                                                    {t("Low quantity")}
                                                </label>
                                        </div>
                                        {Products.isLowQuantity && 
                                        <div className="px-2 py-3">
                                            <input type="number" name="low_quantity" onChange={(e) => setProducts({ ...Products, [e.target.name]: e.target.value })} value={Products?.low_quantity} placeholder="Enter Quantity" className="appearance-none block w-full  text-gray-700 border border-gray-200 rounded py-2 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 width-input-200" />
                                        </div>
                                        }
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center justify-end p-1 border-t border-solid border-slate-200 rounded-b">
                                    ...
                                    {!isSave ?
                                        <button className="bg-emerald-500 text-white active:bg-emerald-600 text-sm px-4 py-2 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button" onClick={updateProduct} > {t("Update")}</button>
                                        :
