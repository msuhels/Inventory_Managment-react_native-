...
import { ADMIN_ADD_USER } from '../../../api/apiUrl';
import toast, { Toaster } from 'react-hot-toast';
import { Spinner } from "reactstrap";
import { useTranslation } from "react-i18next";

export default function Modal({ showModal, setShowModal, getUsers }) {
    let { admin, token } = JSON.parse(sessionStorage.getItem('userDetails'));
    const { t } = useTranslation();
    const [usreDetails, setUsreDetails] = useState({ name: '' });
    ...
    const addUser = async (event) => {
        ...
        setIsSave(true);
        try {
            const response = await axios.post(ADMIN_ADD_USER, usreDetails, { headers: { "Authorization": "Bearer " + token } });
                            ...
                            <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                                <form onSubmit={addUser}>
                                    <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                                        <h3 className="text-3xl font-semibold text-center"> {t("Add User")}</h3>
                                    </div>
                                    <div className="relative p-6 flex-auto">
                                        ...
                                        <div className="py-2">
                                            ...
                                            <select name="role" onChange={(e) => setUsreDetails({ ...usreDetails, [e.target.name]: e.target.value })} className="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" required >
                                                ...
                                                {admin?.role == 'super admin' && <option value="admin">Admin</option>}
                                            </select>
                                        </div>
                                    </div>
