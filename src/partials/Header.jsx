...
import { UPDATE_LANGUAGE } from '../api/apiUrl';
...
function Header({ sidebarOpen, setSidebarOpen }) {
  ...
  useEffect(() => {
    if (localStorage.getItem('selected_language') || userData?.language) {
      let language = userData?.language ?  userData?.language : localStorage.getItem('selected_language');
  ...
  const selectLanguage = async(e) => {
    try {
      const response = await axios.post(UPDATE_LANGUAGE,{language:e}, { headers: { "Authorization": "Bearer " + token } });
